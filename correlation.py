...
def calculate_correlation(df, col_a, col_b):
    ...
    if len(data) > 1:  # Pearson correlation needs at least two data points
        ...
        print(f"Not enough valid data in columns {col_a} and {col_b} after removing NaN/inf.")
        return None
...
# Print the results
print(f"Correlation between file1 and file2: {corr_1_2}")
print(f"Correlation between file1 and file3: {corr_1_3}")
print(f"Correlation between file2 and file3: {corr_2_3}")
...
# # print(f"The response time per row, including categories, has been saved to {output_file}")
...
#     plt.text(norm_count_1 / 2.2, idx, f'{(norm_count_1 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
#     plt.text(norm_count_1 + norm_count_2 / 1.5, idx, f'{(norm_count_2 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
#     plt.text(norm_count_1 + norm_count_2 + norm_count_3 / 1.6, idx, f'{(norm_count_3 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
#     plt.text(norm_count_1 + norm_count_2 + norm_count_3 + norm_count_4 / 1.5, idx, f'{(norm_count_4 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
#     plt.text(norm_count_1 + norm_count_2 + norm_count_3 + norm_count_4 + norm_count_5, idx, f'{(norm_count_5 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)

#     plt.text(0, idx, f'{norm_percent_1_2:.1f}%', va='center', ha='right', color='black', fontsize=12)
#     plt.text(norm_count_1 + norm_count_2 + norm_count_3 + norm_count_4 + norm_count_5 + 0.5, idx, f'{norm_percent_4_5:.1f}%', va='center', ha='left', color='black', fontsize=12)

#     words = item['category'].split()
#     if len(item['category']) > 18:
...
# plt.box(False)
# plt.tight_layout()
# # plt.savefig('receive_accepted_answer_step3.pdf')
