...
def main():
    ...
    # Check if the request was successful (status code in the range 200-299)
        if response.status_code // 100 != 2:
            logger.info(f"Error: HTTP Status Code {response.status_code}")
        ...
        with open(output_file_path, 'a') as output_file:
            for tag in tags:
                ...
                if question_count is not None:
                    ...
                    logger.info(f'Writing result for tag: {tag}, count: {question_count}')
                else:
                    logger.info(f'Failed to retrieve count for tag: {tag}')
                processed_tags_count += 1
                logger.info(f"Processed {processed_tags_count} out of {len(tags)} tags")
