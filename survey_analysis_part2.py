...
for idx, group in tqdm(enumerate(category_counts.keys()), total=len(category_counts.keys())):
    ...
    # print(group, norm_percent_1_2, norm_percent_4_5)
...
for idx, item in enumerate(sorted_data):
    ...
    plt.text(norm_count_1 / 1.2, idx, f'{(norm_count_1 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
    plt.text(norm_count_1 + norm_count_2 / 2, idx, f'{(norm_count_2 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
    plt.text(norm_count_1 + norm_count_2 + norm_count_3 / 2, idx, f'{(norm_count_3 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
    plt.text(norm_count_1 + norm_count_2 + norm_count_3 + norm_count_4 / 1.6, idx, f'{(norm_count_4 / total_norm_count) * 100:.1f}%', ha='right', color='black', fontsize=label_fontsize)
...
plt.box(False)
plt.tight_layout()
# plt.savefig('normalized_bar_chart_step3.pdf')
plt.show()